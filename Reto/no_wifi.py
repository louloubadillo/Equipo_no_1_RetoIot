...
# HR and SPO2 calculations code by Octavio Navarro and Jorge Rodríguez
# - - -
...
def insert_into_database(heartRate, oxygenSaturation, userID):
...
try:
    cnx = mysql.connector.connect(
        user = "root", 
        password = "mac_15_db", # the secret is out T-T
        port = "3306",
        database = 'reto_iot'
    )
    ...
    while(not inserted):
        try:
            ...
            continue
        if(len(hrValues)==100):
            hr_and_spo2 = calc_hr_and_spo2(irValues, redValues)
            HR2 = analisisHR2(hrValues, miliValues, 1)
...
print("¡Listo! Ya puedes checar el dashboard de Tableau :D")